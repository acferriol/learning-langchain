...
def rerank(query, retrieved_documents):
    ...
    scored_documents = [
        v for _, v in sorted(zip(scores, retrieved_documents), reverse=True)
    ]
...
user_question = "como se llama el premio que se le da a alumnos destacados?"

# Buscar la pregunta del usuario en el índice
retrieved_documents = hybrid_search(user_question, limit=3)
...
SYSTEM_MESSAGE = """
Eres un asistente útil que responde preguntas sobre la resolucion.
